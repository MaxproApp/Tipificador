...
(async function initLogin() {
  ...
  function mostrarApp(sesion) {
    ...
    document.dispatchEvent(new CustomEvent("auth:ok", { detail: sesion }));
  }
...
(function () {
  ...
  function pintarCombo(filtro = "") {
    ...
    comboResults = TIPOS.filter(
      (t) => !f || normTxt(t).includes(f) || normTxt(TIPOS_DESC[t]).includes(f),
    );
    ...
    if (comboIndex >= comboResults.length) comboIndex = comboResults.length - 1;
  ...
  bulkTypeClearBtn.addEventListener("mousedown", (e) => e.preventDefault());
  ...
  function animarContadorCambiado(tipo) {
    const btn = document.querySelector(`.type-btn[data-tipo="${tipo}"] .count`);
    if (!btn) return;
  ...
  async function selectPage(id) {
    ...
    document.querySelectorAll(".thumb-item").forEach((el) => {
      el.classList.toggle("selected", parseInt(el.dataset.pageId, 10) === id);
    });
  ...
  function rotateCurrentPage(delta) {
    ...
    showToast(`↻ Página girada a ${page.rotation}°.`);
  }
  ...
  async function loadFiles(files) {
      ...
      try {
        ...
        for (let i = 1; i <= numPages; i++) {
          ...
          pages.push({
            ...
            // Rotación individual de esta página
            rotation: 0,
    ...
    if (loaded > 0) {
      ...
      showToast(`${loaded} documento(s) cargado(s) (${pages.length} páginas).`);
  ...
  async function generarZip() {
    ...
    if (!factura) {
      ...
      facturaInput.focus();
      showToast("⚠️ Ingresa el número de factura antes de generar el ZIP.");
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < tipos.length; i++) {
        ...
        pagesList.sort((a, b) => a.docId - b.docId || a.pageNum - b.pageNum);
        ...
        for (const p of pagesList) {
          ...
          // Aplicar la rotación realizada por el usuario
          if (p.rotation) {
            copiedPage.setRotation(PDFLib.degrees(p.rotation));
          }
      ...
      zipLoaderFinish(true, `ZIP con ${tipos.length} archivo(s) generado.`);
  ...
  const PRESTADORES = (function () {
    ...
    async function cargar() {
      try {
        ...
        cargado = true;
        console.log(`[PRESTADORES] ${lista.length} prestadores cargados.`);
        return true;
      } catch (e) {
  ...
  async function limpiarTodo() {
    ...
    const ok = await pedirConfirmacion({
      ...
      mensaje: "Se eliminarán todos los PDFs cargados y sus asignaciones.",
  ...
  function actualizarPrestador() {
    ...
    if (p) {
      ...
      inforp.textContent = "⚠️ NIT no encontrado en la lista de prestadores";
